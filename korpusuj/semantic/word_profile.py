...
def compute_word_profile(
    ...
    for row_idx, token_indices in hits_by_row.items():
        ...
        # row_idx pochodzi z wyników wyszukiwania / .search jako gęsty doc_id
        # odpowiadający pozycji dokumentu 0..N-1.
        #
        # Parquet może mieć nieciągły indeks etykietowy, np. [0, 2, 4, 5, ...],
        # więc df.loc[row_idx] jest tutaj błędne. Musimy używać df.iloc.
        try:
            _row_pos = int(row_idx)
            if not (0 <= _row_pos < len(df)):
                logging.warning(
                    "Pomijam trafienie profilu: row_idx poza zakresem pozycyjnym df "
                    "(row_idx=%r, df_len=%s)",
                    row_idx,
                    len(df),
                )
            ...
            row_data = df.iloc[_row_pos]

        except Exception as e:
            logging.warning(
                "Pomijam trafienie profilu: błąd pozycyjnego pobierania row_data "
                "(row_idx=%r, reason=%r)",
                row_idx,
                e,
            )
        ...
        for target_idx in token_indices:
            # Defensive guard: some corpus rows may have inconsistent
            # token-level column lengths. Skip such malformed hits instead
            # of aborting the whole word-profile computation.
            if target_idx < 0 or target_idx >= len(upostags):
                logging.warning(
                    "Pomijam trafienie profilu: target_idx poza zakresem upostags (target_idx=%s, len_upostags=%s)",
                    target_idx,
                    len(upostags),
                )
                continue
