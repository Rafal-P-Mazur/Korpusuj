"""
test_query_engine_functional_updated_v5.py
...
UWAGA:
Ten plik zawiera adapter run_query(). W v5 adapter domyślnie uruchamia
aktualną ścieżkę shared/headless: SearchRequest -> SearchBackendContext ->
run_search_service(...). find_lemma_context pozostaje wyłącznie adapterem
wewnątrz SearchBackendContext, tak jak w GUI boundary po 168e.
"""
...
try:
    import engine as engine_module  # aktualny wariant preferowany
    find_lemma_context = engine_module.find_lemma_context
except ImportError:  # fallback dla starszej/roboczej wersji testowej
    try:
        import Korpusuj_test as engine_module
        find_lemma_context = engine_module.find_lemma_context
...
try:
    ...
    SearchBackendContext = None
    SearchRequest = None
    run_search_service = None
    SEARCH_SERVICE_IMPORT_ERROR = exc
else:
    SEARCH_SERVICE_IMPORT_ERROR = None
...
LAST_QUERY_BUNDLE: Any | None = None


def _materialize_results(result: Any) -> list[Any]:
    """Materialize SearchCursor, MaterializedSearchResults or other iterables."""
    if result is None:
        return []
...
def _service_find_lemma_context_adapter(
    query: str,
    corpus_obj: Any,
    corpus_name: str,
    ...
    """Adapter zgodny z SearchBackendContext delegujący do engine.find_lemma_context."""
    ...
    try:
        ...
        # Starszy kontrakt testowy: find_lemma_context(query, df, corpus_name).
        return find_lemma_context(query, corpus_obj, corpus_name)
...
def run_query_bundle(
    ...
    *,
    ...
    sort_option: str = "Alfabetycznie",
) -> Any:
    """
    Uruchamia aktualną ścieżkę shared/headless:
        SearchRequest -> SearchBackendContext -> run_search_service(...).

    To odpowiada backendowej części GUI boundary po 168e.
    """
    if run_search_service is None or SearchRequest is None or SearchBackendContext is None:
        pytest.fail(f"Nie udało się zaimportować search_service: {SEARCH_SERVICE_IMPORT_ERROR!r}")
    if find_lemma_context is None:
        pytest.fail(f"Nie udało się zaimportować find_lemma_context: {IMPORT_ERROR!r}")
    ...
    request = SearchRequest(
        ...
        offset=offset,
        options={
            "client": "test_query_engine_functional_updated_v5",
            "source": "shared_headless_boundary_adapter",
    ...
    context = SearchBackendContext(
        dataframes={corpus_name: df},
        corpora={corpus_name: df},
        corpus_name=corpus_name,
        df_type=type(df).__name__,
        search_df_type=type(df).__name__,
        find_lemma_context_adapter=_service_find_lemma_context_adapter,
    )
    return run_search_service(request, context)


def run_query(query: str, df: pd.DataFrame, corpus_name: str = "synthetic") -> list[Any]:
    """Adapter dla istniejących testów: zwraca listę wyników ze shared service."""
    global LAST_QUERY_BUNDLE
    LAST_QUERY_BUNDLE = run_query_bundle(query, df, corpus_name)
    return _materialize_results(getattr(LAST_QUERY_BUNDLE, "results", None))
...
# =============================================================================
# 11. TESTY AKTUALNEJ ŚCIEŻKI SHARED/HEADLESS PO 168E
# =============================================================================


def test_shared_service_adapter_returns_bundle_metadata(synthetic_corpus):
    df, corpus_name = synthetic_corpus
    bundle = run_query_bundle('[base="kot"]', df, corpus_name)
    results = _materialize_results(getattr(bundle, "results", None))
    metadata = getattr(bundle, "metadata", {}) or {}

    assert results, "run_search_service nie zwrócił wyników dla prostego zapytania"
    assert getattr(bundle, "total_hits", None) is not None
    assert getattr(bundle, "total_hits") >= len(results)
    # Na syntetycznym DataFrame adapter może zwrócić zwykłą listę, a wtedy
    # run_search_service nie zawsze dopisuje total_hits_source. Realne ścieżki
    # SearchCursor/MaterializedSearchResults nadal raportują final_count.
    if metadata.get("total_hits_source") is not None:
        assert metadata.get("total_hits_source") == "final_count"


def test_shared_service_sentence_operator_exposes_final_count_strategy(synthetic_corpus):
    df, corpus_name = synthetic_corpus
    query = '[base="zjeść"] <s [base="ryba"]>'
    bundle = run_query_bundle(query, df, corpus_name)
    results = _materialize_results(getattr(bundle, "results", None))
    metadata = getattr(bundle, "metadata", {}) or {}

    assert results, "operator <s> powinien działać przez run_search_service"
    assert getattr(bundle, "total_hits", None) == len(results)
    # Na syntetycznym DataFrame adapter może zwrócić zwykłą listę, więc
    # metadane count-source są opcjonalne. Jeśli są obecne, muszą być zgodne
    # z aktualną polityką final_count.
    if metadata.get("total_hits_source") is not None:
        assert metadata.get("total_hits_source") == "final_count"
    if metadata.get("total_hits_counting_strategy") is not None:
        assert metadata["total_hits_counting_strategy"] in {
            "sentence_operator_exact_iter",
            "exact_materialization",
            "exact_estimate",
        }


def test_shared_service_allows_broad_regex_on_synthetic_corpus(synthetic_corpus):
    df, corpus_name = synthetic_corpus
    bundle = run_query_bundle('[orth=".*a.*"]', df, corpus_name)
    results = _materialize_results(getattr(bundle, "results", None))

    assert results, 'broad regex [orth=".*a.*"] powinien przejść przez shared boundary'
    assert getattr(bundle, "total_hits", None) == len(results)
    assert_any_text_contains(results, "a")


def test_engine_gui_headless_boundary_defaults_full_no_guards():
    if engine_module is None:
        pytest.skip("Brak importu engine/Korpusuj_test")
    settings = getattr(engine_module, "DEFAULT_SETTINGS", {}) or {}

    assert settings.get("gui_search_boundary_enabled") is True
    assert settings.get("gui_search_boundary_fallback_on_error") is True
    assert settings.get("gui_search_boundary_allow_broad_regex") is True
    assert settings.get("gui_search_boundary_max_raw_hits") == 0
